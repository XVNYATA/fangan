...
@-moz-document regexp(".*://(?!((www.|m.|)(example.com|example.com|example.com|example.com|example.com|example.com))|v.|([a-z0-9.-_]*/(player|photo|file|u/0/photos|document/d/|spreadsheets/d/|presentation/d/))|(imgur|pixlr|www.youtube|[a-z]{2}.battle.net|[a-z]{2}.blizzard.com).).*") {
html>*,html>* [style*="url"]:not([style*="base64,R0lGODlhAQABAI"]),img:not([class*="ogo"]),embed,object,video,:-webkit-full-screen:not(iframe),iframe:not(:-webkit-full-screen),frame,canvas{-webkit-filter:invert(1)hue-rotate(180deg)!important}
...
.svg-icon:hover, .newUI .entry:hover .svg-icon:hover {
    fill: #fff !important;
}
